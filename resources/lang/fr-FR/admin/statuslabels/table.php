<?php

return array(
    'about'      	=> 'About Status Types',
    'archived'      	=> 'Retiré',
    'create'      	=> 'Créé une étiquette de statut',
    'color'      	=> 'Couleur du graphique',
    'default_label' => 'Libellé par défaut',
    'default_label_help' => 'Ceci est utilisé pour s\'assurer que vos étiquettes de statut les plus couramment utilisées apparaissent en haut de la case de sélection lors de la création/l\'édition des actifs.',
    'deployable'      	=> 'Déployable',
    'info' => 'Les types d\'état servent à décrire les différents états dans lesquels vos actifs peuvent se trouver. Ils peuvent être en réparation, perdus/volés, etc. Vous pouvez créer de nouvelles étiquettes d\'état pour vos actifs déployables, en attente et archivés, en fonction de votre propre flux de travail. Pour plus d\'informations, contactez-nous <a href="https://patrimoinestate.com/contact/" target="_blank"><i class="fa fa-external-link"></i></a>.',
    'name'      	=> 'Nom du statut',
    'pending'      	=> 'En attente',
    'status_type'   => 'Type d\'état',
    'show_in_nav'   => 'Afficher dans la barre latérale',
    'title'      	=> 'Étiquette de statut',
    'undeployable'  => 'Non déployable',
    'update'      	=> 'Mettre à jour l\'étiquette de statut',
);
